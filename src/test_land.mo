...
shared(msg) actor class Token(
    ...
    private stable var androidImages : [Text] = [];
    stable var imageCounter = 0;

    var buffer = B.Buffer<Text>(100);


    stable var counter = 0;
    stable var androidPrincipal : Text = "";
    
    public func setImage(imageText : Text) : async Nat {
                
        buffer.add(imageText);
        //buffer.put(imageCounter,imageText);
        imageCounter += 1;        
        return buffer.size();
    };    

    public query func getImageSize() : async Nat {               
       
        return imageCounter;
    };    


    public func clearImages() : async Nat {
    ...
    public query func getImage(imageId : Nat) : async Text {
        
        if(imageId >= imageCounter)
        {
            // return small 25x25 Red Square
            return "iVBORw0KGgoAAAANSUhEUgAAABkAAAAZCAYAAADE6YVjAAAAJ0lEQVR42mP8z8AARLQFjKOWjFoyasmoJaOWjFoyasmoJaOW0MUSAI4SMeiXzOiKAAAAAElFTkSuQmCC";
        };
        return buffer.get(imageId);
    ...
    public func setPrincipal(_androidPrincipal : Text) : async Text {
        
        androidPrincipal := _androidPrincipal;
        androidPrincipal;
         
    };

    public query func balance(who: Text) : async Nat {

        var whoPrincipal : Principal = Principal.fromText(who);

        var balanceAmount : Nat = _balanceOf(whoPrincipal);
        return (balanceAmount/100000000);
        
    };

    public shared(msg) func redeem(to: Text) : async Nat {
        
        var redeemTo : Principal = Principal.fromText(to);
        var value : Nat = 10000000000;
        ...
        ignore addRecord(
            msg.caller, "👣 Redeem -" # Nat.toText(airdropcounter),
            [
                ("from", #Principal(owner_)),
                ("to", #Principal(redeemTo)),
        ...
        redeemCounter += 1;
        _transfer(owner_, redeemTo, value);
        /***********************************************/

        txcounter += 1;
        
        var balanceAmount : Nat = _balanceOf(redeemTo);
        return (balanceAmount/100000000);
    };
    ...
    private stable var redeemCounter: Nat = 0;
   ...
   public query func http_request(request: HttpRequest) : async HttpResponse {
        ...
        if (path.size() == 0)  {
          ...
          body := Text.encodeUtf8 (
            ...
            "Image Counter:                              " # Nat.toText(imageCounter) #"\n" 
          );

        }  else {

                headers := [("Content-Type", "image/png")];
                body := "";
...
/********************************************************************/
    ...
    */
    system func preupgrade() {
        ...
        androidImages := buffer.toArray();
    };

    system func postupgrade() {

        //buffer := B.fromArray(androidImages);
        androidImages := [];
